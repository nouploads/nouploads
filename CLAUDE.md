...
- **Title**: "[Category] Tools — Free Online [keywords] | NoUploads"
- **Meta description**: One sentence + privacy angle. Under 160 chars.
...
Every public route must include: title, description, canonical URL, Open Graph tags. Use shared helpers from `app/lib/seo/`. Follow existing JSON-LD patterns. Do not hand-roll metadata if a shared helper exists.
...
Export via `buildMeta()`: title "[Tool Name] Online — Free, Private, No Upload | NoUploads", description under 160 chars. Unique per tool.
...
- route registered in `routes.ts` with metadata (title, description, canonical, OG)
