 ...
 *
 * Note: JPEG-embedded PICTs require browser APIs (createImageBitmap) and are
 * not supported in this portable core decoder. Only PackBits PICTs are handled.
 */
export async function decodePict(input: Uint8Array): Promise<ImageData> {
	const bytes = input;
	...
	if (jpegOffset !== -1) {
		throw new Error(
			"This PICT file contains embedded JPEG data, which requires browser APIs to decode. Use the web app decoder for JPEG-embedded PICT files.",
		);
	}
 ...
 */
function decodePackBitsPixMap(data: Uint8Array): ImageData | null {
	...
	for (let i = 0; i < localData.length - 50; i++) {
		const opcode = view.getUint16(i, false);
		if (opcode !== 0x0098 && opcode !== 0x009a) continue;

		try {
			return parsePixMapData(localData, view, i + 2, opcode);
		} catch {}
	}
