 ...
 */
export async function decodeNetpbm(input: Uint8Array): Promise<ImageData> {
	const bytes = input;
...
/** P1: ASCII PBM (1-bit, 1=black 0=white) */
function decodeP1(bytes: Uint8Array): ImageData {
...
/** P2: ASCII PGM (grayscale) */
function decodeP2(bytes: Uint8Array): ImageData {
...
/** P3: ASCII PPM (RGB) */
function decodeP3(bytes: Uint8Array): ImageData {
	const header = parseStandardHeader(bytes, 2, true);
	const values = readAsciiValues(bytes, header.dataStart);
	const { width, height, maxval } = header;
	const pixelCount = width * height;
...
function decodeP4(bytes: Uint8Array): ImageData {
	const header = parseStandardHeader(bytes, 2, false);
...
function decodeP5(bytes: Uint8Array): ImageData {
	const header = parseStandardHeader(bytes, 2, true);
...
function decodeP6(bytes: Uint8Array): ImageData {
	const header = parseStandardHeader(bytes, 2, true);
	const { width, height, maxval, dataStart } = header;
	const pixelCount = width * height;
	const rgba = new Uint8Array(pixelCount * 4);
...
function decodeP7(bytes: Uint8Array): ImageData {
 ...
 */
function decodePfm(bytes: Uint8Array, channels: 1 | 3): ImageData {
