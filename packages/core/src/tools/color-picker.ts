/**
 * Color parsing + format conversion + WCAG contrast helpers. Single
 * source of truth for web and CLI. Uses culori for CSS color parsing /
 * color-space conversions. Sync.
 */
...
export function allFormats(hex: string): Record<ColorFormat, string> {
	...
	for (const { key } of FORMAT_DEFS) result[key] = formatColor(hex, key);
	return result;
}
...
const tool: ToolDefinition = {
	...
	category: "developer",
	description:
		"Parse CSS colors and convert between HEX, RGB, HSL, HSV, HWB, CMYK, LAB, LCH, XYZ, LUV, OKLCH formats.",
	...
	execute: async (input, _options, context) => {
		...
		const hex = parseToHex(text);
		if (!hex) throw new Error(`Could not parse "${text}" as a color`);
		const formats = allFormats(hex);
		context.onProgress?.(100);
		return {
			output: new TextEncoder().encode(JSON.stringify(formats, null, 2)),
			extension: ".json",
			mimeType: "application/json",
			metadata: { hex, ...(formats as unknown as Record<string, unknown>) },
		};
	},
};
