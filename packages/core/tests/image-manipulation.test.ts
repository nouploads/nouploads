...
describe("resize-image tool", () => {
	...
	it("should default output format to input format from JPEG magic bytes", async () => {
		const tool = getTool("resize-image");
		if (!tool) throw new Error("not registered");
	...
	it("should default output format to input format from PNG magic bytes", async () => {
		const tool = getTool("resize-image");
		if (!tool) throw new Error("not registered");
	...
	it("should default output format to input format from WebP magic bytes", async () => {
		const tool = getTool("resize-image");
		if (!tool) throw new Error("not registered");
		// RIFF....WEBP
	...
	it("should honor explicit --format over input sniff", async () => {
		const tool = getTool("resize-image");
		if (!tool) throw new Error("not registered");
		const jpegMagic = new Uint8Array([
