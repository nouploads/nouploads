...
import {
	...
	validateHtml,
	WRAP_OPTIONS,
} from "../processors/html-formatter";
...
<body>
<div class="container"><h1>Hello, world</h1><p>Paste HTML here and click Format.</p></div>
</body>
</html>`;
...
export default function HtmlFormatterTool() {
	...
	const [wrap, setWrap] = useState<string>("80");
	...
	useEffect(() => {
		...
		setStats(computeHtmlStats(input));
	}, [input]);
	...
	const handleFormat = useCallback(async () => {
		...
		try {
			...
			const formatted = await formatHtml(input, {
				indentSize,
				indentChar,
				wrapLineLength: Number(wrap),
			});
			setInput(formatted);
			setValidationError(null);
			setIsValid(true);
	...
	}, [input, indent, wrap, isFormatting]);
	...
	const handleFileUpload = useCallback(
		(e: React.ChangeEvent<HTMLInputElement>) => {
			...
			if (file.size > MAX_HTML_SIZE) {
				setValidationError(
					`File too large (${formatFileSize(file.size)}). Maximum is ${formatFileSize(MAX_HTML_SIZE)}.`,
				);
				setIsValid(false);
				return;
			}
	...
	const wrapLabel = WRAP_OPTIONS.find((o) => o.value === wrap)?.label ?? wrap;
	...
	return (
		<div className="space-y-4">
			...
			<div className="flex flex-wrap items-center gap-3">
				...
				<div className="flex items-center gap-2">
					<label htmlFor="html-wrap" className="text-sm text-muted-foreground">
						Line wrap:
					</label>
					<Select value={wrap} onValueChange={setWrap}>
						<SelectTrigger
							...
							className="w-[140px]"
						>
							<SelectValue>{wrapLabel}</SelectValue>
						</SelectTrigger>
						<SelectContent>
							{WRAP_OPTIONS.map((o) => (
								<SelectItem key={o.value} value={o.value}>
