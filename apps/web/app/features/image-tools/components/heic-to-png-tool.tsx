...
function SingleFileView({
	...
	// Convert HEIC to a displayable preview (use PNG since that's our target)
	useEffect(() => {
		const controller = new AbortController();

		(async () => {
			try {
				const blob = await heicToPng(file, {
...
function BatchView({ files, onReset }: { files: File[]; onReset: () => void }) {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const outputs = await heicToPngBatch(
					files,
					{ signal: controller.signal },
	...
	const downloadAll = useCallback(() => {
		for (const r of successfulResults) {
			...
			a.href = url;
			a.download = toPngFilename(r.inputFile.name);
			...
			{status === "done" && results.length > 0 && (
				<>
					{results.map((r, i) => {
						...
						const outputName = toPngFilename(r.inputFile.name);
...
export default function HeicToPngTool() {
	...
	return (
		<div className="space-y-6">
			<div className="min-h-[460px]">
				...
				{isSingleFile && <SingleFileView file={files[0]} onReset={reset} />}

				{isBatch && <BatchView files={files} onReset={reset} />}
			</div>
		</div>
	);
}
