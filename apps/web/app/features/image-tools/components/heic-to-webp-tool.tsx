...
function SingleFileView({
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const blob = await heicToWebp(file, {
					quality: 1.0,
	...
	const outputFilename = toWebpFilename(file.name);
...
function BatchView({
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const outputs = await heicToWebpBatch(
					files,
	...
	const downloadAll = useCallback(() => {
		for (const r of successfulResults) {
			...
			a.href = url;
			a.download = toWebpFilename(r.inputFile.name);
			...
			{status === "done" && results.length > 0 && (
				<>
					{results.map((r, i) => {
						...
						const outputName = toWebpFilename(r.inputFile.name);
...
export default function HeicToWebpTool() {
