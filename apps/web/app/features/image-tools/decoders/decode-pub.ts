...
/**
 * Decode a Microsoft Publisher PUB file by extracting embedded images.
 *
 * PUB files are OLE2 compound files. This decoder reads the compound file
 * using the cfb library, then scans all stream data for embedded JPEG,
 * PNG, or BMP images. The largest found image is extracted and decoded
 * to RGBA pixels.
 *
 * Full Publisher document rendering is not feasible in-browser.
 */
export async function decodePub(
	input: Blob,
	signal?: AbortSignal,
	...
	try {
		...
		throw new Error(
			"This PUB file could not be opened. It may be corrupted or not a valid OLE2 compound file.",
		);
	}
	...
	if (images.length === 0) {
		throw new Error(
			"This PUB file could not be rendered. No embedded images were found.",
		);
	}
...
async function decodeImageBytesToPixels(
	...
	try {
		...
		throw new Error("Could not decode the embedded image from the PUB file.");
	}
